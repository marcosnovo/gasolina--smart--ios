...
function parseAnagraficaText(text: string): {
  stationMap: Map<string, StationData>;
  skippedNoCoords: number;
  skippedOutOfBounds: number;
} {
  const lines = text.split("\n");
  if (lines.length < 3) throw new Error("Anagrafica CSV too short");
...
function parsePrezziText(
  ...
  const lines = text.split("\n");
  if (lines.length < 3) throw new Error("Prezzi CSV too short");
  ...
  return { pricesMatched, pricesUnmapped, pricesStale, pricesInvalid, pricesOrphan };
}

function buildStationList(stationMap: Map<string, StationData>): {
...
export async function fetchItaly(db: D1Database): Promise<{ count: number; duration: number }> {
  ...
  console.log(
    `[fetcher:IT] Prezzi: ${prezziStats.pricesMatched} matched, ${prezziStats.pricesUnmapped} unmapped, ${prezziStats.pricesStale} stale, ${prezziStats.pricesInvalid} invalid, ${prezziStats.pricesOrphan} orphan`
  );
  ...
  if (stations.length > 0) {
    await saveStations(db, "IT", stations);
  }
...
export async function shouldFetchItaly(db: D1Database, intervalMinutes: number): Promise<boolean> {
  const last = await getCountryMetaValue(db, "IT", "last_fetch");
  if (!last) return true;
