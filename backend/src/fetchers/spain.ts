...
export async function fetchSpain(): Promise<{ count: number; duration: number }> {
...
export function getSpainLastFetchTime(): string | null {
  return getCountryMetaValue("ES", "last_fetch");
}

export function shouldFetchSpain(intervalMinutes: number): boolean {
  const last = getSpainLastFetchTime();
  if (!last) return true;
