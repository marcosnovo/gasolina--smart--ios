...
export async function fetchFromMinisterio(): Promise<{
  count: number;
  duration: number;
}> {
  const start = Date.now();
  console.log("[fetcher] Starting fetch from Ministerio API...");
  ...
  console.log(
    `[fetcher] Saved ${stations.length} stations in ${duration}ms`
  );
